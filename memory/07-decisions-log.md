...
### 2026-05-03 — Added `Prototype` phase + `mockup-factory` skill + `/mm-mockup` + workflow 07 (iterative full-app design before MVP)
- **Decision:** Add `Prototype` as a formal canonical phase between `Definition` and `MVP` (taking MASTERMIND from 6 to 7 canonical phases). Build `mockup-factory` skill with 3 modes (create / iterate / freeze), platform-aware from day 1 (reads memory/14 §Platform). Build `/mm-mockup` command wrapping it. Build workflow `07-full-app-prototyping.md` documenting the 10-phase end-to-end process. Update `phase-gate-reviewer` with new transitions (`Definition → Prototype`, `Prototype → MVP`) and a skip rule for non-UI projects. Update all documentation surfaces (CLAUDE.md, README.md, COMMANDS.md, OPERATING-GUIDE.md, skill-creator, memory-updater, index READMEs, memory/13-phase-history.md canonical definitions).
- **Reason:** User confirmed they always iterate 2–5 times on design before starting MVP build on any UI-heavy project. Previously this iteration was hidden inside `Definition`, causing three pathologies: (1) `Definition` became conflated with "design complete" — fuzzy gates; (2) stakeholders didn't know when their feedback was most wanted; (3) MVP started on ambiguous design foundations, causing expensive rework. A dedicated phase with its own gate fixes all three and aligns the memory/13 timeline with the lived reality of the user's work. Non-UI projects (backend, CLI, library) skip the phase at no cost with `--skip-reason "no UI"`.
- **Alternatives considered:**
  - Keep `Prototype` as an implicit sub-step of `Definition` — rejected: loses temporal precision, doesn't give stakeholders a clear signal when to engage, doesn't warrant a gate.
  - Put mockup work inside the first sprint of MVP — rejected: conflates validation with construction; post-mockup "this isn't what we wanted" becomes expensive because MVP has already started.
  - Build `mockup-factory` as part of `prototype-designer` with a `--full-app` flag — rejected: the mental model diverges too much (single-feature during build vs whole-product before build); one skill with two contradictory jobs is worse than two focused skills.
  - Auto-deploy the mockup to Vercel/EAS as part of the skill — rejected for v1: out of scope. Deploy instructions are in the workflow; automation is a follow-up if it proves valuable.
- **Consequences:**
  - New skill `.cursor/skills/mockup-factory/SKILL.md` (~320 lines). Platform-aware (web = shadcn/ui + Vercel preview; mobile = RNR + Expo Go). 3 modes + status mode. Invoked by `/mm-mockup`.
  - New command `.claude/commands/mm-mockup.md` (~60 lines) with preconditions + 4 subcommands (create / iterate / feedback / freeze / status).
  - New workflow `.claude/workflows/07-full-app-prototyping.md` (~240 lines) covering 10 internal phases (gate entry → scope → v1 → stakeholder review → iterate → freeze → gate exit → planner handoff → optional deploy → close).
  - Modified `.cursor/skills/phase-gate-reviewer/SKILL.md` with new transitions `Definition → Prototype` and `Prototype → MVP`, plus explicit skip rule for non-UI projects. Added "Entered Prototype" recommended-command block pointing to `/mm-mockup create`.
  - Modified `memory/13-phase-history.md` canonical phase definitions: inserted `Prototype` between `Definition` and `MVP` with entry/exit criteria and artefact list. Added taxonomy-change entry at top of Entries section explaining the 6→7 phase migration.
  - Modified `.claude/workflows/04-phase-gate-transition.md`: description + triggers + applicable_phases list + handoff options now include Prototype.
  - Modified `.cursor/skills/skill-creator/SKILL.md`: total skill count 21→22; `mockup-factory` added to the System 1 Design & prototyping sub-bucket alongside `prototype-designer` with differentiation.
  - Modified `.cursor/skills/memory-updater/SKILL.md`: callers list gains `mockup-factory`; note about freeze-mode multi-section consolidation of memory/14 with per-entry approval.
  - Updated `.claude/workflows/README.md` and `.claude/commands/README.md` indexes with the new workflow 07 and command `/mm-mockup`.
  - Updated `CLAUDE.md`, `README.md`, `COMMANDS.md`, `OPERATING-GUIDE.md` with new counts (22 skills, 14 commands, 7 workflows) and new phase mentions in lifecycle diagrams.
  - Zero breaking changes: existing projects continue on the 6-phase model until they explicitly adopt Prototype. The APP ARMARIO sync-from-template will present Prototype as an opt-in via the new skill/command/workflow.
- **Files affected:**
  - New: `.cursor/skills/mockup-factory/SKILL.md`, `.claude/skills/mockup-factory/SKILL.md` (sync), `.claude/commands/mm-mockup.md`, `.claude/workflows/07-full-app-prototyping.md`.
  - Modified: `memory/13-phase-history.md`, `.cursor/skills/phase-gate-reviewer/SKILL.md` (+ mirror), `.claude/workflows/04-phase-gate-transition.md`, `.cursor/skills/skill-creator/SKILL.md` (+ mirror), `.cursor/skills/memory-updater/SKILL.md` (+ mirror), `.claude/workflows/README.md`, `.claude/commands/README.md`, `CLAUDE.md`, `README.md`, `COMMANDS.md`, `OPERATING-GUIDE.md`, `memory/07-decisions-log.md` (this entry).
  - Plan reference: `.cursor/plans/2026-05-03-add-prototype-phase-and-mockup-factory.md`.
