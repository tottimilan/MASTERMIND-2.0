---
name: mockup-factory
description: Produce a full-app iterative navigable mockup before MVP build starts. Platform-aware: reads memory/14-design-system.md §Platform and branches between web (Next.js + shadcn/ui + browser preview + optional Vercel URL) and mobile (Expo + react-native-reusables + NativeWind + Expo Go on-device preview). Three modes: `create` (v1 from scratch using all flows in memory/05 + all MVP features in memory/06 + all tokens and mobile-specific sections in memory/14), `iterate` (accept stakeholder feedback → produce v2 / v3 / ... / vN), and `freeze` (declare the current version the design-final, copy to docs/design/mockups/final/, consolidate decisions back into memory/14, log transition in memory/13-phase-history.md). Invoked as the working skill of the `Prototype` canonical phase, between Definition and MVP. Different from prototype-designer: prototype-designer prototypes ONE feature before implementing it (inside an MVP / Iteration phase); this skill prototypes the ENTIRE product interactively before any MVP code is written. Use when entering the Prototype phase via /mm-gate Prototype, or when the user says "mockup the full app", "let's design the whole product", "mm-mockup create", "iterate on the mockup", "freeze the design". Requires the design system installed (via scripts/install-shadcn-mcp, platform-aware), memory/14 with Platform + identity + tokens filled, memory/05 flows populated, memory/06 feature-map populated, and Claude Design access.
---
